...
原因：所有 active agent 都运行在 main 分支的工作目录中（`/Users/huayang/code/agents/`）。如果直接在这里修改代码，会导致：
...
这会在 `/Users/huayang/code/agents-dev-mcp-refactor/` 创建一个独立的工作目录，切换到 `feature/mcp-refactor` 分支。

### Step 2: 在 Worktree 中开发

```bash
cd /Users/huayang/code/agents-dev-<feature>
# 正常开发、编辑代码、运行测试
```
...
在**生产目录**（`/Users/huayang/code/agents/`）拉取最新代码并安全重启：
...
| 禁止 | 原因 |
|------|------|
| 在 `/Users/huayang/code/agents/` 直接编辑代码 | 影响运行中的 agent |

...
# In Docker container, path is /app/src/management/instance_manager.py (parents[3] = /app)
# On host, path is deeper (parents[4] = repo root)
_file_path = Path(__file__).resolve()
_default_root = str(_file_path.parents[min(4, len(_file_path.parents) - 1)])
REPO_ROOT = Path(os.environ.get("MGMT_REPO_ROOT", _default_root))

# When running in Docker with host docker.sock, the build context paths in generated
# docker-compose.yml must be HOST paths (Docker daemon runs on host, not in container).
# MGMT_HOST_REPO_ROOT should be set to the repo path on the host machine.
HOST_REPO_ROOT = os.environ.get("MGMT_HOST_REPO_ROOT", str(REPO_ROOT))
...
def _generate_compose_yaml(slug: str, web_port: int) -> str:
    ...
    Uses HOST_REPO_ROOT for build context paths because Docker daemon
    runs on the host (via docker.sock), not inside the management container.
    """
    repo = HOST_REPO_ROOT
    return dedent(f"""\
        ...
        services:
          daemon:
            build:
              context: {repo}/services/agents-mcp
            ports:
...
# Host-side instances dir — for docker compose --project-directory (Docker daemon is on host)
HOST_INSTANCES_DIR = Path(os.environ.get("MGMT_HOST_INSTANCES_DIR", str(INSTANCES_DIR)))


def _get_instance_dir(slug: str) -> Path:
    """Get the instance directory path (inside container)."""
    return INSTANCES_DIR / slug


def _get_host_instance_dir(slug: str) -> Path:
    """Get the instance directory path on the host (for docker compose commands)."""
    return HOST_INSTANCES_DIR / slug
...
async def _run_compose(slug: str, *args: str, timeout: int = 120) -> subprocess.CompletedProcess:
    ...
    The compose file is read from the container-internal path,
    but --project-directory uses the HOST path so that Docker daemon
    (running on host via socket) can resolve relative volume mounts.
    """
    container_dir = _get_instance_dir(slug)
    host_dir = _get_host_instance_dir(slug)
    project_name = _get_project_name(slug)

    cmd = [
        "docker", "compose",
        "--project-name", project_name,
        "-f", str(container_dir / "docker-compose.yml"),
        "--project-directory", str(host_dir),
        *args,
    ]

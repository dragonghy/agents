...
**优先选择 Web 项目**，因为它们可以部署到 Vercel 让任何人在线访问和测试。在创建 ticket 时，明确说明项目类型（Web/CLI/Mobile）和期望的部署方式。
...
- **Bug / Blocker**：创建 ticket，assignee 设为 Product（用 `suggest_assignee(role="product")`）

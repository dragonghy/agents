...
1. **立即 commit 代码**（强制，不可跳过）：
   - 主分支开发：`git add <files> && git commit -m "feat/fix: <description>"`
   - Worktree 开发：commit 后立即 merge 回 main
   - **禁止**：改完代码不 commit 就交付给 QA。未 commit 的代码 = 未交付。

2. **在当前 ticket 上添加完成备注**（包含 commit hash）：
   ```
...
3. **将当前 ticket 交接给 QA**：
   ...
   reassign_ticket(
     ...
     comment="开发完成，请验收。交付内容和测试要点见上方备注。commit <hash>。"
   )
   ```

4. **然后查询是否有新的待办任务**（status=3 或 status=4），有则继续执行。
...
使用 DEPENDS_ON 模式（详见 `/tasks` 手册"阻塞等待"章节）：
...
- 消息适用于快速沟通；正式的工作交付仍通过 ticket 管理。

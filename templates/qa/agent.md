...
2. **Commit 检查**：Dev 交付时必须附带 commit hash。如果 Dev 备注中没有 commit hash，**打回给 Dev 要求先 commit 代码再交付**。未 commit 的代码不接受测试。
3. **需求一致性检查**：对比交付物与 Product Spec，如果 Dev 擅自改动了需求，必须指出并上报 Product。

...
def setup_skills(agent_name, agent_dir, source_templates_dir, base_name=None):
    ...
    # Symlink instance-specific skills (override template skills if same name)
    if base_name and base_name != agent_name:
        instance_skills_dir = os.path.join(source_templates_dir, agent_name, "skills")
        if os.path.isdir(instance_skills_dir):
            for skill_name in sorted(os.listdir(instance_skills_dir)):
                skill_src = os.path.join(instance_skills_dir, skill_name)
                if os.path.isdir(skill_src):
                    force_symlink(skill_src, os.path.join(skills_dir, skill_name))
...
# This directory is auto-generated by setup-agents.py

**Do not modify scaffolding files here** (CLAUDE.md, .claude/, .mcp.json).
Changes will be overwritten on the next run of `python3 setup-agents.py`.

## How to make changes

| What to change | Where to edit |
|---|---|
| Agent template (CLAUDE.md) | `templates/<role>/CLAUDE.md` |
| Role-specific skills | `templates/<role>/skills/` |
| Shared skills | `templates/shared/skills/` |
| Agent definition (system prompt) | `.claude/agents/<role>.md` |

After editing templates, run `python3 setup-agents.py` to regenerate.

## Safe to use directly

- `journal/` — Runtime data (daily journals, notes)
- Any files you create that aren't part of the scaffolding
"""


def generate_agent_marker(agent_dir):
    """Write GENERATED.md warning file into an agent workspace directory."""
    marker_path = os.path.join(agent_dir, "GENERATED.md")
...
def setup_all(cfg, config_path, source_dir, output_dir):
    ...
    for name, info in agents_expanded.items():
        ...
        if is_instance:
            ...
            if os.path.isfile(agent_specific_claude):
